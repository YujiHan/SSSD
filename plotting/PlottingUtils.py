...
def computeVisEmbedding(true_data, model_pred_data, embed_name):
    if embed_name == "umap":
        true_umap_traj, umap_model = umapWithoutPCA(
            np.concatenate(true_data, axis=0), n_neighbors=50, min_dist=0.2
        )
        model_pred_umap_traj = [
            umap_model.transform(np.concatenate(m_pred, axis=0))
...
def computeLatentEmbedding(latent_seq, next_seq, n_neighbors, min_dist):
    latent_tp_list = np.concatenate(
        [np.repeat(t, each.shape[0]) for t, each in enumerate(latent_seq)]
    )
    umap_latent_data, umap_model = umapWithoutPCA(
        np.concatenate(latent_seq, axis=0), n_neighbors=n_neighbors, min_dist=min_dist
    )
    # umap_latent_data, umap_model = onlyPCA(np.concatenate(latent_seq, axis=0), pca_pcs=2)
    umap_latent_data = [
        umap_latent_data[np.where(latent_tp_list == t)[0], :]
        for t in range(len(latent_seq))
    ]
