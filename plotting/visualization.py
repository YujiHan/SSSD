...
# ======================================
image_path = '/home/hanyuji/Workbench/SSSD2/version2/temp.png'
...
def plotPredAllTime(true_umap_traj, pred_umap_traj, true_cell_tps, pred_cell_tps):
    ...
    for i, t in enumerate(unique_tps):
        ...
        ax1.scatter(
            true_umap_traj[true_t_idx, 0],
            true_umap_traj[true_t_idx, 1],
            label=t,
            color=color_list[i],
            s=20,
            alpha=1.0,
        ...
        ax2.scatter(
            pred_umap_traj[pred_t_idx, 0],
            pred_umap_traj[pred_t_idx, 1],
            label=t,
            color=color_list[i],
            s=20,
            alpha=1.0,
...
def plotPredTestTime(
    true_umap_traj, pred_umap_traj, true_cell_tps, pred_cell_tps, test_tps
):
...
def compareUMAPTestTime(
    ...
    ax_list[0].scatter(
        true_umap_traj[:, 0], true_umap_traj[:, 1], c=gray_color, s=40, alpha=0.5
    )
    ...
    for m_idx, m in enumerate(model_list):
        ...
        for t_idx, t in enumerate(test_tps):
            ...
            ax_list[m_idx + 1].scatter(
                model_pred_umap_traj[m_idx][pred_t_idx, 0],
                model_pred_umap_traj[m_idx][pred_t_idx, 1],
                label=int(t),
                color=c,
                s=20,
                alpha=0.9,
...
def plotMetricBar(
    ...
    for data_name in dataset_list:
        ...
        metric_res = np.load(
            "{}/{}-{}-model_metrics.npy".format(metric_path, data_name, inter_split),
            allow_pickle=True,
        ).item()
        tmp_ot = np.asarray(
            [
                [
                    metric_res[t][m]["global"]["ot"] if m in metric_res[t] else np.nan
                    for m in inter_model_list
                ]
                for t in metric_res
            ]
        )
        tmp_l2 = np.asarray(
            [
                [
                    metric_res[t][m]["global"]["l2"] if m in metric_res[t] else np.nan
                    for m in inter_model_list
                ]
                for t in metric_res
            ]
        ...
        metric_res = np.load(
            "{}/{}-{}-model_metrics.npy".format(metric_path, data_name, extra_split),
            allow_pickle=True,
        ).item()
        tmp_ot = np.asarray(
            [
    ...
    ax_list[0].set_xticks(
        np.arange(len(dataset_list)) - inter_width * len(inter_model_list) / 2
    )
    ...
    plt.xticks(
        np.arange(len(dataset_list)) - extra_width * len(extra_model_list) / 2,
        [dataset_name_dict[x] for x in dataset_list],
    )
    plt.xlabel("Dataset")
...
def _plotTable(df_data):
    print(
        tabulate.tabulate(
            df_data,
            headers=[df_data.index.names[0] + '/' + df_data.columns.names[1]]
...
def printMetric(model_l2, model_ot, model_list, column_names):
    ...
    metric_df = pd.DataFrame(
        ...
        index=pd.MultiIndex.from_tuples(
            [("L2", m) for m in model_list], names=("Metric", "Model")
        ),
    ...
    metric_df = pd.DataFrame(
        data=model_ot,
        index=pd.MultiIndex.from_tuples(
            [("OT", m) for m in model_list], names=("Metric", "Model")
        ),
        columns=pd.MultiIndex.from_tuples(column_names, names=("Type", "TP")),
    )
    _plotTable(metric_df)
...
def computeDrift(traj_data, latent_ode_model):
    ...
    drift_seq = [
        latent_ode_model.computeDrift(each_latent).detach().cpu().numpy()
        for each_latent in latent_seq
    ]
    latent_seq = [each_latent.detach().cpu().numpy() for each_latent in latent_seq]
...
def plotStream(
    ...
    X, Y = np.meshgrid(
        np.linspace(min_X, max_X, num_sep), np.linspace(min_Y, max_Y, num_sep)
    )
    ...
    U, V = velocity[:, 0].reshape(num_sep, num_sep), velocity[:, 1].reshape(
        num_sep, num_sep
    )
    ...
    plt.streamplot(
        ...
        color="k",
        linewidth=1.0,
        start_points=grid_coord[selected_idx],
    )
    plt.show()
    plt.savefig(image_path)


def plotStreamByCellType(
    umap_scatter_data,
    umap_latent_data,
    umap_next_data,
    traj_cell_type,
    ...
    for i, c in enumerate(np.unique(concat_cell_types)):
        ...
        plt.scatter(
            concat_umap_scatter[c_idx, 0],
            concat_umap_scatter[c_idx, 1],
            color=color_list[i] if c != "NAN" else gray_color,
            s=10,
            alpha=0.6,
        )
